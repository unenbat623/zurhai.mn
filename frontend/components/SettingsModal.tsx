...
export default function SettingsModal({ isOpen, onClose, userProfile, onUpdateProfile }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 p-0 backdrop-blur-xl md:p-6">
      <motion.div
        initial={{ opacity: 0, scale: 0.95, y: 20 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        className="glass-card relative flex h-full w-full max-w-4xl flex-col overflow-hidden border-white/10 bg-slate-900 shadow-2xl md:h-[600px] md:flex-row"
      >
  ...
  return (
    <div className="group flex items-start justify-between gap-4">
      ...
      <button 
        ...
        className={`flex h-6 w-12 shrink-0 items-center rounded-full px-1 transition-all ${checked ? 'bg-indigo-600' : 'bg-slate-800'}`}
      >
        <motion.div 
