  ...
  return (
    <div className="mx-auto max-w-4xl py-6 sm:py-8">
      ...
      <div className="flex justify-center mb-10">
        <div className="flex w-full max-w-md gap-1 rounded-2xl border border-white/5 bg-slate-900/50 p-1 sm:w-auto">
          <button
            onClick={() => setMode('general')}
            className={`min-w-0 flex-1 rounded-xl px-3 py-2.5 text-[10px] font-bold uppercase tracking-widest transition-all sm:px-8 ${mode === 'general' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-slate-500 hover:text-white'}`}
          >
            Ерөнхий Тохироо
          </button>
          <button
            onClick={() => setMode('daily')}
            className={`min-w-0 flex-1 rounded-xl px-3 py-2.5 text-[10px] font-bold uppercase tracking-widest transition-all sm:px-8 ${mode === 'daily' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-slate-500 hover:text-white'}`}
          >
            Өнөөдрийн Энерги
          </button>
        </div>
      ...
      <div className="flex justify-center mb-16">
        <button
          ...
          className="flex w-full max-w-sm items-center justify-center gap-3 rounded-2xl bg-indigo-600 px-6 py-4 text-xs font-bold uppercase tracking-widest text-white shadow-lg shadow-indigo-600/20 transition-all hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 active:scale-95 disabled:opacity-50 sm:w-auto sm:px-12"
        >
          <Heart size={18} />
      ...
      <div aria-live="polite" aria-atomic="true">
        ...
        {activeResult && !loading && (
          <motion.div
            ...
            className="glass-card relative overflow-hidden border-indigo-500/20 bg-gradient-to-br from-indigo-900/30 to-slate-900 p-5 sm:p-8 md:p-12"
          >
            ...
            <div className="text-center mb-16">
                ...
                <AnimatePresence>
                  {activeResult.score > 70 && (
                    <motion.div 
                      ...
                      animate={{ opacity: 1 }}
                    >
                      {[-84, -48, -18, 24, 56, 92].map((offset, i) => (
                        <motion.div
                          key={i}
              ...
              <p className="mx-auto max-w-2xl text-base font-light italic leading-relaxed text-slate-400 md:text-lg">
                "{activeResult.summary}"
              </p>
            </div>

          <div className="grid grid-cols-1 gap-8 border-t border-white/5 pt-8 md:grid-cols-2 md:gap-12">
            <div>
  ...
  return (
    <div className="glass-card p-4 md:p-8 overflow-x-auto">
      <div className="min-w-[600px]">
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-3 text-[9px] font-bold uppercase tracking-widest text-slate-500 md:gap-6">
...
function SignSelector({ label, selected, onSelect }: { label: string, selected: ZodiacSign | null, onSelect: (s: ZodiacSign) => void }) {
  return (
    <div className="glass-card border-white/5 bg-slate-900/30 p-5 sm:p-8">
      ...
      <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 sm:gap-3">
        {ZODIAC_SIGNS.map(s => (
          <button
            ...
            } min-h-11 rounded-xl text-xs font-bold uppercase tracking-tighter`}
          >
